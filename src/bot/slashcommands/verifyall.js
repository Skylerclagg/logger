...
module.exports = {
  ...
  func: async interaction => {
    ...
    guild.members.forEach(member => {
      ...
      // Initiate verification: here we send a DM prompting verification.
      member.user.getDMChannel()
        .then(dm => dm.createMessage('Please verify by replying with your full name.'))
        .catch(err => console.error(err));
      count++;
    });
    return interaction.createMessage({ content: `Started verification process for ${count} members.`, flags: Eris.Constants.MessageFlags.EPHEMERAL });
  }
};
